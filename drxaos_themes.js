...
/* ===================================================================
   ...
   body.drxaos-low-perf .card.focus {
       box-shadow: inset 0 -4px 0 0 var(--drxaos-accent-color) !important; /* Only bottom line */
...
body.drxaos-xuyampishe-active {
    --drxaos-xu-hero-base-height: clamp(320px, 36vw, 520px);
...
function drxaosSetActiveLine(activity, line) {
    ...
    function drxaosHandleCardActivation(target) {
        // Navigation Guard: Record activity to pause background tasks
        window.drxaosLastNavigationTime = Date.now();
        
        if (!drxaosIsXuyampisheEnabled() || !target || drxaosIsFocusLockActive()) return;
        ...
        if (card && drxaosIsValidCard(card) && card.classList && card.classList.contains('focus')) {
            ...
            }, drxaosIsTvDevice() ? 50 : 10); // Reduced latency: 50ms for TV, 10ms for PC
        }
    }
        ...
        if (window.jQuery) {
            this.html = window.jQuery(
                ...
                '<div class="drxaos-xu-info__body">' +
                    ...
                    '<div class="drxaos-original-title-block drxaos-xu-info__tags drxaos-hero-tags hero drxaos-original-title-block--empty"></div>' +
                '</div>' +
            '</div>'
            );
        } else {
    ...
    DrxaosXuyampisheInfo.prototype.draw = function(payload, cardElement) {
        ...
        var cardId = drxaosResolveCardId(payload);
        var originalBlock = drxaosEnsureHeroOriginalTitleBlock(root);
        var prelimOriginal = drxaosBuildOriginalTitleTemplate(drxaosComposeOriginalTitleData(payload, cardElement));
        if (originalBlock) {
            originalBlock.innerHTML = prelimOriginal || '';
            originalBlock.classList.toggle('drxaos-original-title-block--empty', !prelimOriginal);
        }
        if (cardId) {
            var heroType = payload && payload.name ? 'tv' : 'movie';
            drxaosFetchOriginalNames({ id: cardId }, heroType).then(function(result) {
                if (!result) return;
                var tpl = drxaosBuildOriginalTitleTemplate(result);
                if (!tpl) return;
                var blk = drxaosEnsureHeroOriginalTitleBlock(root);
                if (!blk) return;
                blk.innerHTML = tpl;
                blk.classList.toggle('drxaos-original-title-block--empty', !tpl);
            }).catch(function() {});
        }
            ...
            var state = {
                ...
                updateBackground: function(data) {
                    ...
                    background.style.opacity = '1';
                        ...
                        background.src = path;
                    // Обработчики для случаев ошибки загрузки
    ...
    currentTheme = 'midnight';
        ...
        function focusThemeItem($item, options) {
            ...
            if ($item[0] && typeof $item[0].scrollIntoView === 'function') {
                ...
                $item[0].scrollIntoView({ behavior: options.instant ? 'auto' : (preferSmoothScroll ? 'smooth' : 'auto'), block: 'nearest', inline: 'center' });
            }
        }
...
function addSettings() {
    try {
        ...
        if (!Lampa.Storage.get('jacred_url')) {
            drxaosSafeSet('jacred_url', 'jacred.xyz');
        }
...
function applyLoader(loaderId) {
    try {
        var loaders = {
            ...
            'loader5': '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"><style>@keyframes loader15{0%{transform:scale(.2);opacity:.8}80%{transform:scale(1.2);opacity:0}to{transform:scale(2.2);opacity:0}}</style><circle cx="12" cy="12" r="6" fill="#FF0000" style="animation:loader15 1.5s cubic-bezier(.165,.84,.44,1) infinite both;transform-origin:center center"/><path fill="#FF0000" fill-rule="evenodd" d="M13.41 11.591a.5.5 0 010 .817l-1.73 1.222a.5.5 0 01-.788-.409v-2.442a.5.5 0 01.789-.409l1.729 1.222z" clip-rule="evenodd"/></svg>',
...
function processAllCards() {
    ...
    function processCardList(list, batchSize, budget, isHighPriority, onComplete) {
        ...
        function runBatch() {
            // Navigation Guard: Pause heavy processing while user is navigating (FPS Protection)
            if (window.drxaosLastNavigationTime && (Date.now() - window.drxaosLastNavigationTime < 300)) {
