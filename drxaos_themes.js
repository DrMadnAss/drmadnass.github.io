...
function showUnknownQualityBadge(element) {
    ...
    function drxaosIndexLines(activity) {
        ...
        lines.forEach(function(line, idx) {
            ...
            if (!isMadness && !line.classList.contains('drxaos-line-active')) {
                line.style.display = 'none';
            } else if (isMadness && line.style.display === 'none') {
                line.style.display = '';
            }
        });
    ...
    try {
        ...
        loaderItems.forEach(function(item) {
            ...
            item.addEventListener('mouseenter', function() {
                ...
                item.style.background = 'rgba(51,65,85,0.95)';
            });
            item.addEventListener('mouseleave', function() {
                item.style.borderColor = 'rgba(148,163,184,0.3)';
                item.style.background = 'rgba(15,18,22,0.95)';
            });
        });
...
function processAllCards() {
    ...
    var BATCH_SIZE = 3; // Обрабатываем по 3 карточки за раз для лучшей отзывчивости
    var currentIndex = 0;
    
    function processBatch() {
        var endIndex = Math.min(currentIndex + BATCH_SIZE, cards.length);
        var batch = cards.slice(currentIndex, endIndex);
...
function drxaosRenderHeroTitle(hero, payload, fallbackTitle, cardId) {
    ...
    // Если лого нет в кеше - показываем текст маленьким размером
    titleNode.textContent = safeFallback;
    titleNode.classList.remove('drxaos-hero-title-has-logo');
    
    // Загружаем лого асинхронно для следующего раза
    ...
    drxaosLoadTitleLogo(normalized).then(function(result) {
        ...
        // Ошибка загрузки - оставляем текст как есть
    });
}
