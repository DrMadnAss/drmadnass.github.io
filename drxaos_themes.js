...
body.drxaos-madness-mode .card:not(.madness-card):not(.madness-item):not(.madness__item):not([data-madness-card]):not([data-component="madness-card"]) {
    display: none !important;
}
...
body.drxaos-xuyampishe-active .card:not(.card--wide):not(.madness-card):not(.madness-item):not(.madness__item):not([data-madness-card]):not([data-component="madness-card"]) {
    display: none !important;
}
...
var CONFIG = {
    ...
    function drxaosEnsureBadgeDefaults() {
        ...
        DRXAOS_BADGE_OPTIONS.forEach(function(option) {
            ...
            if (existing == null) {
                Lampa.Storage.set(storageKey, option.default || 'on');
            }
        });
    }
    ...
    function drxaosApplyBadgeSettings() {
        try {
            ...
            logError('Error applying badge settings', err);
        }
        try {
            applySeasonInfo();
        } catch (applyErr) {
            logError('applySeasonInfo failed after badge settings', applyErr);
        }
        if (typeof window.drxaosProcessAllCards === 'function') {
            setTimeout(window.drxaosProcessAllCards, 60);
        }
    }
...
function addSettings() {
    ...
    try {
        ...
        paramsFailed++;
        logError('Error adding badge toggle settings', badgeSettingsError);
    }
    Lampa.SettingsApi.addParam({
        component: 'drxaos_themes',
        param: {
...
function drxaosCleanupSeasonBadges() {
    ...
    ['.card--content-type', '.card__type', '.card--season-complete', '.card--season-progress', '.card__seasons', '.card-seasons', '.card--country', '.card__country', '.card--year', '.card__year', '.card__date', '.card-year', '.card__release-year'].forEach(function(selector) {
...
`;
        ...
        if (!badgeTypeEnabled) {
            drxaosRemoveBadgeNodes(cardEl, (DRXAOS_BADGE_LOOKUP.type && DRXAOS_BADGE_LOOKUP.type.selectors) || ['.card--content-type']);
        }
        if (!badgeCountryEnabled) {
            drxaosRemoveBadgeNodes(cardEl, (DRXAOS_BADGE_LOOKUP.country && DRXAOS_BADGE_LOOKUP.country.selectors) || ['.card--country']);
        }
        if (!badgeSeasonEnabled) {
            drxaosRemoveBadgeNodes(cardEl, (DRXAOS_BADGE_LOOKUP.season && DRXAOS_BADGE_LOOKUP.season.selectors) || ['.card--season-complete', '.card--season-progress']);
        }
        if (!badgeYearEnabled) {
            drxaosRemoveBadgeNodes(cardEl, (DRXAOS_BADGE_LOOKUP.year && DRXAOS_BADGE_LOOKUP.year.selectors) || ['.card--year']);
        }
...
function drxaosRenderYearBadge(cardElement, yearText) {
    ...
    drxaosRemoveBadgeNodes(cardElement, (DRXAOS_BADGE_LOOKUP.year && DRXAOS_BADGE_LOOKUP.year.selectors) || ['.card--year']);
    var badge = document.createElement('div');
...
function drxaosSyncMadnessMode() {
    try {
        ...
        logError('Error syncing madness mode flag', err);
    }
}
    ...
    if (window.drxaosGlobalObserver) {
        window.drxaosQualityHandler = function(mutations) {
            ...
            if (hasNewCards) {
                ...
                setTimeout(drxaosProcessAllCards, 150);
            }
        };
    } else {
        var observer = new MutationObserver(function(mutations) {
